...
const CartPage = () => {
  const {
    ...
    isLoggedIn,
    refreshApiCart,
    mergeCartOnLogin, // Make sure this is implemented in your cart context
  } = useCart();
  ...
  // Sync cart on login and initial load
  useEffect(() => {
    const syncCart = async () => {
      try {
        if (isLoggedIn) {
          // Merge local and server carts when logged in
    ...
    syncCart();
  }, [isLoggedIn]); // Only runs when login status changes
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                {isLoggedIn && (
                  <div className="bg-blue-50 p-4 text-blue-600 text-sm border-b border-blue-100">
                    <p>Your cart is synced with your account</p>
                  </div>
                )}
